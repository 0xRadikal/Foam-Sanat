...
describe('productSchema', () => {
  ...
  it('rejects emoji media with non-emoji characters', () => {
    expect(() =>
      productSchema.parse({
        slug: 'invalid-emoji',
        status: ProductStatus.DRAFT,
        ...
        descEn: 'd',
        priceMode: PriceMode.UNAVAILABLE,
        media: [{ type: ProductMediaType.EMOJI, emoji: 'abc', sortOrder: 0 }],
      }),
    ).toThrow();
  });

  it('allows non-fixed price modes without amount', () => {
    const result = productSchema.parse({
      slug: 'price-contact',
      status: ProductStatus.DRAFT,
      titleFa: 'fa',
    ...
    expect(result.priceMode).toBe(PriceMode.CONTACT);
